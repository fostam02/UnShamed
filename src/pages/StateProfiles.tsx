...
export const StateProfiles = () => {
  ...
  const getLicensesByStateId = (stateId: string): License[] => {
    ...
    // Get associatedLicenseId from state or state.metadata
    const associatedLicenseId = state.associatedLicenseId || state.metadata?.associatedLicenseId;

    // If associatedLicenseId is not defined, return empty array
    if (!associatedLicenseId) {
      // If user has a license for this state, return it
      const stateAbbr = state.abbreviation?.toLowerCase();
      if (stateAbbr && userProfile.licenses) {
        const matchingLicenses = userProfile.licenses.filter(
          license => license.state?.toLowerCase() === stateAbbr
        );
        if (matchingLicenses.length > 0) return matchingLicenses;
      }
      return [];
    }

    // Check if the associatedLicenseId is a comma-separated string of IDs
    if (typeof associatedLicenseId === 'string' && associatedLicenseId.includes(',')) {
      const licenseIds = associatedLicenseId.split(',');
      return licenseIds
      ...
      const license = getLicenseById(associatedLicenseId);
  ...
  const getPrimaryContactName = (state: any) => {
    try {
      // Check in both state and state.metadata
      const complianceMonitor = state.complianceMonitor || state.metadata?.complianceMonitor;
      const investigator = state.investigator || state.metadata?.investigator;

      if (complianceMonitor?.name) {
        return {
          name: complianceMonitor.name,
          role: 'Compliance Monitor'
        };
      } else if (complianceMonitor?.monitorName) {
        return {
          name: complianceMonitor.monitorName,
          role: 'Compliance Monitor'
        };
      } else if (investigator?.name) {
        return {
          name: investigator.name,
          role: 'Investigator'
        };
      } else if (state.boardContact) {
        return {
          name: typeof state.boardContact === 'string' ? state.boardContact : 'Board Contact',
          role: 'Board Contact'
        };
      }
    } catch (error) {
      console.error('Error getting primary contact name:', error);
    }
    return null;
  };
                          ...
                          <Link to={`/state/${state.id}`}>
                            <div className="border rounded-lg p-4 hover:bg-accent/40 transition-colors">
                              ...
                              <div className="space-y-3 mt-4">
                                ...
                                {(state.initialResponseDueDate || state.metadata?.initialResponseDueDate) && (
                                  <div className="flex items-center text-sm">
                                    <Calendar className="h-4 w-4 mr-2 text-muted-foreground" />
                                    <span className="text-muted-foreground mr-1">Response Due:</span>
                                    <span className="font-medium">
                                      {new Date(state.initialResponseDueDate || state.metadata?.initialResponseDueDate || '').toLocaleDateString()}
                                    </span>
                                  </div>
                                )}
